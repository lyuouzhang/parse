...
#'
#' @usage parse_large(tuning, K = NULL, lambda = NULL, y, N = 100, kms.iter = 100, kms.nstart = 100,
#'       eps.diff = 1e-5, eps.em = 1e-5, model.crit = 'gic', backward = TRUE, cores=2)
#' parse_large(tuning = NULL, K, lambda, y, N = 100, kms.iter = 100, kms.nstart = 100,
#'       eps.diff = 1e-5, eps.em = 1e-5, model.crit = 'gic', backward = TRUE, cores=2)
#'
...
#' output <- parse_large(K = c(1:2), lambda = c(0,1), y=y, cores=2)
#' output$mu.hat.best
#' @keywords external
#'
#' @export
...
parse_large <- function(tuning=NULL, K=NULL, lambda = NULL, y, info_gene, N = 100, kms.iter = 10,
                        kms.nstart = 10, eps.diff = 1e-5, eps.em = 1e-5, model.crit = 'gic', backward = TRUE,
                        cores = 2, min.iter = 10){
  ## to deal with large data
  ## replace matrix calculation by marginal
  ...
  for(j.tune in 1:dim(tuning)[1]){
    ...
    else if(lambda1 == 0)
    {
      ...
      kms1 = kmeans(y[,info_gene], centers = K1, nstart = kms.nstart, iter.max = kms.iter)
      ...
      for(k in 1:K1){
        if(length(y[kms1.class == k, 1]) == 1) {
          ## if there is only one data in a cluster,
          mu[1,k,] <- y[kms1.class == k,]
        }
        else {
          mu[1,k,] <- colMeans(y[kms1.class == k,])
        }
      }
      ...
      for (t in 1:(N-1)) {
        ...
        if(t>min.iter){
          ...
          parse.positive = which(apply(abs(parse.diff),2,sum)>1e-3)
        }else{
          parse.positive = 1:d
        }
        p0 = length(parse.positive)

        temp.normal = matrix(0,n,K1)
        for(j in 1:p0){
          tmp = matrix(0,n,K1)
          for(k in 1:K1){
            tmp[,k] = foreach(i = 1:n, .combine = c)%do%{
              dnorm(y[i,j], mu[t,k,j], sigma.iter[t,j], log=TRUE)
            }
          }
          temp.normal = temp.normal + tmp/p0
        }
        temp.normal[which(temp.normal==Inf)] = 1e6
        ...
        sigma.iter[(t+1),] = sigma.iter[(t),]
        sigma.iter[(t+1),] <- sapply(parse.positive, sig.fn, all.combined=all.combined, alpha=alpha.temp[(t+1),,])

        mu[(t+1),,] = mu[(t),,]
        for(j in parse.positive){
          mu[(t+1),,j] <- parse_backward(j=1, y=y[,j], mu.all = mu[t,,j], alpha=alpha.temp[(t+1),,],
                                         sigma.all = sigma.iter[(t+1),j], eps.diff = eps.diff, K=K1, lambda = lambda1,
                                         optim.method ='BFGS')
        }
