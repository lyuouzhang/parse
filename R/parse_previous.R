...
parse_previous <- function(tuning=NULL, K=NULL, lambda = NULL, y, N = 100, kms.iter = 100, kms.nstart = 100,
  ...
  for(j.tune in 1:dim(tuning)[1]){
    ...
    if(K1 == 1) {
      ...
      if(d<=3000){
        ...
        kms1 = kmeans(y0[,1:20], centers = K1, nstart = kms.nstart, iter.max = kms.iter)
      }
      kms1.class = kms1$cluster

      mean0.fn <- function(k){
        if(length(y[kms1.class == k, ]) == 1) {
