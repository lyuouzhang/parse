...
# alternative without sigma
obj_parse_fn_alt <- function(mu.j, loc, y.j, alpha, K, lambda, eps.diff = 1e-5){
  ## mu.j = distinct values (in original order)
  ## loc = which clusters have same mean, e.g. c(1,1,2,2,3) means C1=C2, C3=C4
  n = length(y.j)
  if(length(mu.j) == 1){        ## penalty = 0
...
parse_backward_single <- function(y, mu.all, alpha, K, lambda, eps.diff = 1e-5, optim.method = 'BFGS'){
  ...
  for(q in 1:(K-1)){
    ...
    for(i in 1:dim(combin1)[2]){
      ...
      out1 <- optim(mu.init1[1:(K-q)], obj_parse_fn_alt, method = optim.method, loc = temp.loc[i,], y.j = y, alpha = alpha, eps.diff = eps.diff, K = K, lambda = lambda)
      temp.obj[i] <- out1$value
      temp.mu[i,] <- out1$par[temp.loc[i,]]
    }
    obj.value[q+1] <- min(temp.obj)
